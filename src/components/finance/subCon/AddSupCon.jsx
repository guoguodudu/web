...
import {checkNull,successInfo,errorInfo,initLegalInfo,initSupCode,checkIsNull,checkNumNull,isGreaterToday,warningInfo} from '../../../Common.jsx';
import {selectUtils} from '../../../SelectUtils.jsx';
import moment from 'moment';
import {checkIsNum} from "../../../Common";
const dateFormat = 'YYYY-MM-DD';
class AddSupCon extends React.Component {
    ...
    componentWillMount() {
        console.log("----------AddSupCon.componentWillMount---------------");
        this.canAjax = true;
    }

    componentWillReceiveProps(nextProps) {//每次重新打开此组件，需重新初始化值
        console.log("----------AddSupCon.componentWillReceiveProps---------------",nextProps);
        if(nextProps.visiable){
            this.setState({
                ...
                legalCode:undefined,
                supCode:undefined,
                supConCode:undefined,
                supConStartDate:undefined,
                supConEndDate:undefined,
                supConStartDateStr:undefined,
                supConEndDateStr:undefined,
                supPayPeriod:undefined,
                accName: undefined,
                accBank: undefined,
                accNo: undefined,
            });
    ...
    handleOk = async() => {
        console.log("------AddSupCon----submit------提交数据到后台---state--------------",this.state);
        ...
        if(!checkIsNull(result) && !checkIsNull(result.data) && result.data.result === 1){
            ...
            this.state.parent.setState({addSupConVisiable:false})
    ...
    handleCancel = () => {
		...
		this.props.parent.setState({addSupConVisiable:false})		
    };
