...
class EditSup extends React.Component {
    ...
    componentWillMount() {
        console.log("------EditSup----componentWillMount---------------");
    }

    componentDidMount() {
        console.log("------EditSup----componentDidMount---------------");
    }

    componentWillReceiveProps(nextProps) {//每次重新打开此组件，需重新初始化值
        console.log("------EditSup----componentWillReceiveProps---------------",nextProps);
    ...
    handleCancel = () => {
        ...
        this.props.parent.setState({EditSupVisible:false})
    };
    ...
    handleOk = async() => {
        ...
        if(checkResult){ // 所有校验通过
            ...
            let result = await supInfo.update(obj)
                .catch(function (error) {//防止报错
                    console.log(error)
                });
