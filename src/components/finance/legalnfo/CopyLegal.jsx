...
class CopyLegal extends React.Component {
    ...
    componentWillMount() {
        console.log("------CopyLegal------componentWillMount---------------");
    }

    componentWillReceiveProps(nextProps) {//每次重新打开此组件，需重新初始化值
        console.log("------CopyLegal-----nextProps--componentWillReceiveProps---------------",nextProps);
    ...
    handleOk = async() => {
        ...
        if(!checkIsNull(result) && !checkIsNull(result.data) && result.data.result === 1){
            ...
            this.state.parent.setState({copyLegalVisiable:false})
    ...
    handleCancel = () => {
		...
		this.props.parent.setState({copyLegalVisiable:false})
    };
