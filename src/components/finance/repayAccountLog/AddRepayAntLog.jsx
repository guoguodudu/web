...
class AddRepayAntLog extends React.Component {
    ...
    componentWillMount() {
        console.log("----------AddRepayAntLog.componentWillMount---------------");
        this.canAjax = true;
    }

    componentWillReceiveProps(nextProps) {//每次重新打开此组件，需重新初始化值
        console.log("----------AddRepayAntLog.componentWillReceiveProps---------------",nextProps);
        if(nextProps.visiable){
            this.setState({
                parent:nextProps.parent,
                visiable:nextProps.visiable,
                supCode:undefined,
                repayAccName:undefined,
                repayAccBank:undefined,
                repayAccNo:undefined,
                remarks:undefined,
            });
    ...
    handleOk = async() => {
        console.log("------AddRepayAntLog----submit------提交数据到后台---state--------------",this.state);
        ...
        let result = await repayAccountLog.add(data)
        ...
        if(!checkIsNull(result) && !checkIsNull(result.data) && result.data.result === 1){
            ...
            this.state.parent.setState({addRepayAccountLogVisiable:false})
    ...
    handleCancel = () => {
		...
		this.props.parent.setState({addRepayAccountLogVisiable:false})		
    };
