...
class CopyRepayAntLog extends React.Component {
    ...
    componentWillMount() {
        console.log("------CopyRepayAntLog------componentWillMount---------------");
    }

    componentWillReceiveProps(nextProps) {//每次重新打开此组件，需重新初始化值
        console.log("------CopyRepayAntLog-----nextProps--componentWillReceiveProps---------------",nextProps);
    ...
    handleOk = async() => {
        console.log("------CopyRepayAntLog----submit---提交数据到后台---state--------------",this.state);
        ...
        if(!checkIsNull(result) && !checkIsNull(result.data) && result.data.result === 1){
            ...
            this.state.parent.setState({copyRepayAccountLogVisiable:false})
    ...
    handleCancel = () => {
		...
		this.props.parent.setState({copyRepayAccountLogVisiable:false})
    };
...
export default CopyRepayAntLog;